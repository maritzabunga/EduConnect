...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nama) || empty($email) || empty($password)) {
        ...
        if ($result->num_rows > 0) {
            ...
            // Insert data (Default status='Siswa', foto='img/avatar.png')
            $stmt = $conn->prepare("INSERT INTO users (nama_pengguna, email, password, status, foto) VALUES (?, ?, ?, 'Siswa', 'img/avatar.png')");
            $stmt->bind_param("sss", $nama, $email, $hashed_password);

            if ($stmt->execute()) {
                $success = "Pendaftaran berhasil! Silakan login.";
                // ALUR: Redirect ke login.php setelah 1.5 detik
                echo "<script>
                        setTimeout(function() { 
...
<head>
    ...
    <style>
        ...
        .success-msg { color: #10b981; font-size: 14px; margin-top: 10px; text-align: center; font-weight: bold;}
    </style>
</head>
<body>
